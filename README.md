...
Modern LLM-based deobfuscators can reverse simple identifier renaming and string encoding, so the strongest configurations **layer** transforms (string array + mangling + structural + honeypots) and lean on structural/visual approaches rather than renaming alone. Where renaming is used, `--poison-names` turns it from a no-op (an LLM just re-derives meaningful names) into a trap that anchors the cleanup pass on misleading names. Benchmark the presets with `cargo bench`, and measure how much each transform actually reduces non-JS extraction with `cargo test --test efficacy` (a differential harness that runs a DOM-aware extractor before/after: `--structural` drops recall to ~0.06, cosmetic transforms are friction only at ~1.0).
