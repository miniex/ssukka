...
- **Split into a Cargo workspace** of thin front-ends over one engine: `ssukka_core` (engine lib - offline, `forbid(unsafe)`), `ssukka` (facade lib that re-exports core + the CLI bin, so `use ssukka::*` and `cargo install ssukka` are unchanged), `ssukka-proxy` (serve-time reverse-proxy binary, skeleton), and `ssukka_wasm` (wasm-bindgen `cdylib`, built only for `wasm32`). The wasm bindings now also expose the AI opt-out transport helpers (`content_usage_header`, `robots_txt`, `well_known_tdmrep_json`). (`Cargo.toml`, `ssukka_core/`, `ssukka/`, `ssukka-proxy/`, `ssukka_wasm/`)
