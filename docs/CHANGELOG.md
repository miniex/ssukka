...
- **`--vm` control-flow virtualization** (`Obfuscator::virtualize`): a stronger alternative to `--cff` over the same straight-line blocks - lift the statements into an op table of arrow-function thunks plus an XOR-encoded bytecode array run by a generic dispatch loop, so the execution order lives in data (the bytecode) rather than in JS control flow. Correct-by-construction (same eligible subset; arrow thunks keep `this`/`arguments`); when both flags are set `--vm` wins. Verified by the OBsmith Node harness (which now runs both the flattening and virtualization paths). (`ssukka_core/src/js_ast.rs`)
- **`--tdm-policy <url>` / `Obfuscator::tdm_policy`**: add a TDMRep `tdm-policy` `<meta>` linking a machine-readable usage policy alongside `tdm-reservation`; implies `--ai-opt-out`. (`ssukka_core/src/{config,transform,obfuscator}.rs`)
- **`ssukka-proxy` serve-time reverse proxy**: forwards to an HTTP origin, obfuscates HTML responses polymorphically (per-request), stamps the AIPREF `Content-Usage` header, and serves `/robots.txt` + `/.well-known/tdmrep.json` from the opt-out helpers. The engine stays offline; the proxy is the networked host. Published to crates.io (`cargo install ssukka-proxy`) and wired into the Dockerfile (`--target ssukka-proxy`), the Nix flake (`nix build .#ssukka-proxy`), and `deploy.sh`. (`ssukka-proxy/`)
- **Property-key transformation** (`--property-keys`, `property_keys(true)`): convert object-literal keys to computed string keys (`{foo: 1}` -> `{["foo"]: 1}`), so with the string array the key names are hoisted and encoded instead of left in plain sight. Skips methods, getters/setters, shorthand, already-computed keys, numeric keys, and `__proto__`. Implies `--js-ast`. (`ssukka_core/src/js_ast.rs`)
- **String-array knobs** (`--reserved-strings`, `--string-array-threshold`; `reserved_strings(...)`, `string_array_threshold(...)`): a whitelist of strings to keep readable (for reflection/`eval`) and a fraction-encoded threshold to trade coverage for size/speed. (`ssukka_core/src/js_ast.rs`)
- **OBsmith-style semantics tests** (`ssukka_core/tests/obsmith.rs`): execute representative snippets under Node before and after the full JS pipeline and assert identical output, catching semantic deviations (not just parse failures); skipped when `node` is unavailable. (`ssukka_core/tests/obsmith.rs`)
- **Execution lock** (`--domain-lock <hosts>`, `--lock-expiry <unix-secs>`; `domain_lock(...)`, `lock_expiry_secs(...)`): inject a guard that crashes the script (unbounded recursion -> uncaught `RangeError`) when run off an allowed host (or subdomain) or past an expiry, so the obfuscated code can't be lifted to another site or kept working forever. A no-op on the allowed domain before expiry; random identifiers avoid a fixed signature. A deterrent, not DRM (stripping the guard defeats it). Implies `--js-ast`. (`ssukka_core/src/js_ast.rs`)
- **Opaque predicates** (`--opaque-predicates`, `opaque_predicates(true)`): wrap top-level expression statements in always-true guards (`if(<opaque>){ stmt }`) built from bitwise identities that hold for every non-negative int32 input (`a^b == (a|b)-(a&b)`, `a&b <= a|b`, `a|b >= a`), putting real code behind a condition a static/LLM pass must evaluate. Declarations and string-literal directives are left unwrapped. Implies `--js-ast`. (`ssukka_core/src/js_ast.rs`)
- **MBA (mixed boolean-arithmetic)** (`--mba`, `mba(true)`): replace integer literals with equivalent bitwise/arithmetic expressions (`a+b == (a^b)+2*(a&b) == (a|b)+(a&b)`, exact for int32-range operands in JS), so a static or LLM pass must do the arithmetic to read the constant. Numeric property keys and out-of-range/float literals are left alone. Implies `--js-ast`. (`ssukka_core/src/js_ast.rs`)
- **Readability-tier efficacy tests** (`ssukka_core/tests/extraction.rs`, dev-dep `dom_smoothie` - a Mozilla Readability port): confirm `--structural` starves a real extractor (recall ~0) and the readability tier drops hidden decoys. A naive-tier test in `ssukka_core/tests/efficacy.rs` shows `--structural` + `--honeypots` _poisons_ a visibility-blind scraper (real-body recall ~0.09 while it harvests decoy filler).
- **Editable decoy/word lists in `assets/`** via `ssukka::wordlist::parse` + `include_str!` (comma-separated, `#`-commented). Embedded at build time: editable without touching source, no runtime I/O (offline/WASM-safe). (`ssukka_core/src/wordlist.rs`, `ssukka_core/assets/honeypot/*.txt`)
- **AI opt-out transport helpers** (`ssukka::ai_opt_out`): standalone functions for the canonical AIPREF/TDMRep transports an HTML library can't emit itself — `robots_txt()` (a site-wide AIPREF `Content-Usage: train-ai=n` rule plus a `Disallow` group per known AI training crawler), `content_usage_header()` (the HTTP response-header value), and `well_known_tdmrep_json()` (the `/.well-known/tdmrep.json` body, with optional `tdm-policy` URL) — for edge/server (e.g. Cloudflare Worker) deployment. Targeted crawler tokens are exposed as `AI_TRAINING_CRAWLERS` and the tracked AIPREF vocabulary draft as `AIPREF_VOCAB_DRAFT`. (`ssukka_core/src/ai_opt_out.rs`)

### Changed

- **`--comment-split` now fragments long runs throughout** (about one marker per 6 chars) instead of once per word, so long words and space-free scripts like CJK are split at several points rather than a single one. (`ssukka_core/src/word_split.rs`)
- **Control-flow flattening is now a real state-machine dispatcher** (`--cff`). It rewrites straight-line sequences - the top level and function/arrow block bodies - into a `while`/`switch` with shuffled cases, a random state variable, and label-based transitions, hoisting `var` bindings. Correct-by-construction: a block flattens only when every statement is an expression statement or a simple `var` declaration (no control flow, `let`/`const`, or declarations; `this`/`arguments` untouched), and chosen blocks are non-overlapping. Verified by the OBsmith Node harness. (`ssukka_core/src/js_ast.rs`)
- **Split into a Cargo workspace** of thin front-ends over one engine: `ssukka_core` (engine lib - offline, `forbid(unsafe)`), `ssukka` (facade lib that re-exports core + the CLI bin, so `use ssukka::*` and `cargo install ssukka` are unchanged), `ssukka-proxy` (serve-time reverse-proxy binary), and `ssukka-wasm` (wasm-bindgen `cdylib`, built only for `wasm32`). The wasm bindings also expose the AI opt-out transport helpers. The Dockerfile now has `ssukka` / `ssukka-proxy` / `wasm` targets, the Nix flake gained a wasm dev toolchain and a `ssukka-proxy` package, and `deploy.sh` builds the proxy + wasm artifacts. (`Cargo.toml`, `ssukka_core/`, `ssukka/`, `ssukka-proxy/`, `ssukka-wasm/`, `Dockerfile`, `flake.nix`, `tools/deploy.sh`)
- **Polymorphic mode now deepens the JS chain** (`--polymorphic` with `--js-ast`). On each seedless build it stacks a random mix of the semantics-preserving AST transforms (mangle, MBA, opaque predicates, property-keys, dead code, string array) on top of what you enabled - never dropping a transform you chose - so each build is a different, deeper chain (chaining is where LLM cleanup degrades). (`ssukka_core/src/obfuscator.rs`)
- **Watermark is now redundant and error-correcting** (`--watermark`). The id is scattered as up to 8 copies across separate text nodes and recovered by majority vote over every complete frame, so it survives partial deletion (truncation, copying one section) and bit corruption - not just an intact copy-paste. Still pure zero-width (renders identically); a Unicode normalizer that strips the zero-width class still defeats it. (`ssukka_core/src/watermark.rs`, `ssukka_core/src/transform.rs`)
- **Honeypot decoy blocks now emit dense, link-free article-like prose** instead of short random tokens (`--honeypots`), wrapped in a content-like class (`article-body`, ...). An extractor starved of the real body by `--structural` scores the decoy as the article and harvests the filler (data poisoning, not just wasted effort). Still hidden, marked, and removed on load for JS clients. (`ssukka_core/src/honeypot.rs`)
- **AI opt-out signals are now standards-aligned** (`--ai-opt-out`, `emit_ai_opt_out(true)`). The injected `<head>` `<meta>` block adds W3C **TDMRep** `tdm-reservation` (the EU CDSM Art.4 / AI Act rights-reservation lane) and a best-effort IETF **AIPREF** `Content-Usage: train-ai=n` (`http-equiv`) alongside the legacy `robots: noai, noimageai`. The block is centralized in the new `ssukka::ai_opt_out::meta_block` (with optional `tdm-policy` URL). (`ssukka_core/src/ai_opt_out.rs`, `ssukka_core/src/transform.rs`)
